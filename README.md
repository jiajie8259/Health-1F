# 健康記事（家人健康紀錄）

Apple Health 風格的家庭健康紀錄網站。時間軸／清單雙模式、全站搜尋、標籤與治療狀態、附件上傳、GPT 對話匯入。純前端 + Firebase，可直接部署在 GitHub Pages。

---

## 這一版做了什麼、還沒做什麼

**已完成（核心功能 + GPT 匯入 + 健保快易通匯入 + 健康狀況病症追蹤）**
- 「健康狀況」病症追蹤（首頁）：可建立長期病症（白內障、痔瘡…），底下累積多筆紀錄、寫整體病程說明，一鍵「整理給醫生看」
- 「健保快易通」資料匯入：支援門診資料、用藥資料、影像或病理檢查資料、檢驗檢查結果（HTML 匯出檔），自動整理成時間軸、可分辨已存在資料避免重複匯入、自動遮蔽報告內姓名等識別資料、英文報告盡量附上中文參考翻譯
- 「家人」欄位：每筆紀錄可標記為媽媽／爸爸／其他，時間軸、清單、篩選都支援
- 時間軸 / 清單雙模式切換
- 新增、編輯、刪除紀錄（分類、標籤、治療中／追蹤中／已完成狀態）
- 全站即時搜尋
- 附件上傳（PDF／照片／報告）→ 壓縮後以 base64 存進 **Firestore**（不是 GitHub 倉庫、也不是 Firebase Storage，原因見下方），完全在免費 Spark 方案內，不需要信用卡
- GPT 對話匯入頁：可貼上文字，或上傳 ChatGPT 匯出的 `conversations.json`，自動解析成候選紀錄，逐筆確認後再匯入
- 敏感資料自動遮蔽：偵測到疑似身分證字號、電話號碼格式時，會自動用 █ 遮蔽
- 簡單密碼保護（家人共用一組密碼）

**這版還沒做（下一階段可以再幫你加）**
- 一鍵匯出成 PDF／JSON 備份
- 紀錄之間的「關聯」標記（例如同一症狀的多次回診串在一起）
- 真正的帳號系統（目前是「一組密碼＋瀏覽器記住的名字」，不是每個人獨立登入）

---

## 健康狀況頁怎麼用

這是網站打開後的**預設首頁**，用來追蹤「長期病症」（跟時間軸不一樣：時間軸是一筆一筆的日期事件，健康狀況是把多筆事件串成一個病症的完整病程）。

1. 選好媽媽／爸爸，按「＋ 新增病症」，輸入名稱（例如「白內障」「痔瘡」「走路困難」）、狀態、整體說明
2. 點進病症卡片，可以：
   - 「＋ 新增紀錄」直接加一筆歸屬這個病症的紀錄
   - 「掛入既有紀錄」把時間軸上已經存在、但還沒歸屬任何病症的舊紀錄勾選加進來——標題跟病症名稱相符的會自動勾選、標「建議」，其餘可以自己搜尋勾選
   - 「📋 整理給醫生看」把這個病症的完整病程（發現時間、最近狀態、逐次紀錄、曾用藥物）整理成一段文字，可以直接複製貼給醫生看或照著唸
3. 新增/編輯一般紀錄時，表單多了「歸屬病症」選填欄位，可以直接把新紀錄掛進某個病症

**跟舊版「GPT對話紀錄」病症索引的差異**：側邊欄那個是從標題文字反推猜的分類，沒辦法直接管理；現在這個是你自己建立、可以編輯名稱/狀態/說明的真正病症物件，側邊欄的「病症快速索引」也改成列出這些真正的病症，點了會直接跳到健康狀況頁。

## 為什麼附件是存進 Firestore，不是 GitHub 倉庫、也不是 Firebase Storage？

**不用 GitHub 倉庫**：GitHub Pages 是純靜態網站，網頁要「寫入」GitHub 倉庫必須帶著一組 GitHub API Token 一起送出請求。這組 Token 只能寫在前端程式碼裡，代表任何人打開瀏覽器開發者工具都看得到，並能拿去改你的倉庫。對健康相關檔案來說這風險太高。

**不用 Firebase Storage**：原本這是最直覺的做法，但 Google 從 **2026 年 2 月 3 日起**規定，只要使用 Cloud Storage for Firebase，專案就必須升級到 **Blaze 付費方案**（需連結一組有效的信用卡），即使實際用量是 0 元也一樣，Spark 免費方案完全無法使用 Storage。既然你想維持免費帳號，這條路就不能走了。

**改用 Firestore 存 base64**：附件在瀏覽器端先壓縮（照片會自動縮小、降畫質），轉成 base64 文字後，當成一筆獨立的 Firestore 文件存起來，跟記錄本身用 `recordId` 關聯。Firestore 的 Spark 免費額度（1GB 儲存空間、每天 5 萬次讀取、2 萬次寫入）完全不用信用卡就能用，對家庭用量來說很充裕。

**代價**：
- 單一附件上限約 700KB（壓縮後），超過會提示你先壓縮再上傳
- 大量高解析度照片會比較快用完 1GB 額度（不過一般家庭使用要存好幾千張才會滿）
- PDF 檔案沒辦法自動壓縮，如果原始檔太大，需要自行用其他工具（例如手機內建的「檔案」App 或線上壓縮工具）先縮小再上傳

---

## 關於「密碼保護」的真實安全等級（請務必閱讀）

這個 App 沒有使用 Firebase Authentication（帳號登入），而是用一組「全家共用密碼」，密碼的雜湊值存在 Firestore 的 `settings/access` 文件裡，網頁核對雜湊後才顯示內容。

這種做法可以：
- ✅ 擋住不小心點到網址、隨手滑到的外人
- ✅ 不用管理多組帳號，家人共用一組密碼即可

**沒辦法**：
- ❌ 擋住刻意想繞過的人 —— 因為 Firebase 專案的設定（apiKey 等）本來就會出現在前端程式碼裡，這是所有純前端 + Firebase 網站的共同限制。只要有心人拿到這組設定，就有機會繞過網頁介面直接呼叫 Firestore。

所以這是「避免家人以外的人不小心看到」等級的保護，**不建議記錄身分證字號、健保卡號等高度敏感資料**（App 也會自動幫忙遮蔽偵測到的格式）。如果未來想要更高等級的保護（例如每人獨立帳號登入），可以再升級成 Firebase Authentication，屆時我可以幫你調整。

---

## 建置步驟

### 1. 建立 Firebase 專案（不需要信用卡，Spark 免費方案即可）
1. 前往 [Firebase Console](https://console.firebase.google.com/)，建立新專案
2. 左側選單 → **Firestore Database** → 建立資料庫（正式環境模式即可，規則下面會提供）——**不需要**建立 Storage
3. 左側選單 → 專案設定（齒輪圖示）→ 一般 → 往下滑到「你的應用程式」→ 新增網頁應用程式
4. 複製產生的設定物件，貼到 `js/firebase-config.js` 裡的 `firebaseConfig`

### 2. 設定 Firestore 安全規則
Firestore → 規則，貼上：

```
rules_version = '2';
service cloud.firestore {
  match /databases/{database}/documents {
    match /records/{recordId} {
      allow read, write: if true;
    }
    match /conditions/{conditionId} {
      allow read, write: if true;
    }
    match /attachments/{attachmentId} {
      allow read, write: if true;
    }
    match /settings/{docId} {
      allow read: if true;
      allow write: if !exists(/databases/$(database)/documents/settings/$(docId));
      // 密碼只能在第一次「尚未設定」時寫入，之後透過網頁的密碼比對邏輯更新
    }
  }
}
```

> 這組規則刻意只開放 `records`、`conditions`、`attachments`、`settings` 四個集合，不是整個資料庫全開，降低風險範圍。

### 3. 部署到 GitHub Pages
1. 在 GitHub 建立新的 repository（可設定為 Private，多一層保護）
2. 把 `health-journal` 資料夾內所有檔案上傳（`index.html`、`css/`、`js/`）
3. Repository → Settings → Pages → Source 選擇要發布的分支（例如 `main`）與資料夾（`/root`）
4. 存起來後，GitHub 會給一個網址，例如 `https://你的帳號.github.io/倉庫名稱/`
5. 打開網址，第一次會要求「設定密碼」，設定完成後全家用同一組密碼登入即可

### 4. 首次使用
- 第一次進入會請你「設定密碼」（全家共用）
- 進入後會請你輸入「稱呼」（存在自己瀏覽器的 localStorage，用來標記是誰新增的紀錄，每個人在自己的裝置上輸入一次即可）
- 側邊欄「鎖定畫面」可以隨時登出目前裝置

---

## GPT 對話匯入怎麼用

匯入頁支援**兩種**檔案格式：

**格式一：ChatGPT 官方匯出檔**
1. 到 ChatGPT 網頁版 → 設定 → 資料控制 → 匯出資料，會收到一封信附上下載連結，解壓縮後有一個 `conversations.json`
2. 拖進匯入頁，系統會用關鍵字（症狀、看診、檢查、用藥相關字詞）掃過你（user）發出的訊息，挑出看起來跟健康有關的段落整理成候選紀錄

**格式二：自訂的「依家人分類」結構化 JSON**（例如 `{ "people": { "mother": {...}, "father": {...} } }`，每人底下有 `timeline` 陣列）
- 系統會自動偵測這種格式，直接讀取每一筆的 `symptoms`／`medical_findings`／`treatment`／`status` 等欄位，不需要關鍵字猜測，準確度更高
- 會自動依 `mother`／`father` 分配「家人」欄位；日期若只精確到月或年（如 `"2026-04"`），會補成月初並在描述開頭提醒你確認
- `status` 對應：`completed`→已完成、`follow_up`／`observation`→追蹤中、`pending`→治療中

不論哪種格式，每張候選卡片都可以：勾選要不要匯入、調整家人／日期／分類／狀態／標籤。確認沒問題後按「儲存已勾選的紀錄」。

**注意事項**：ChatGPT 格式是關鍵字比對，不是真的 AI 語意理解，所以：
- 可能會漏掉沒有包含關鍵字、但其實是健康相關的內容
- 也可能誤判一些無關內容
- 系統只是幫你「先篩一輪、省打字」，**請務必逐筆檢查再匯入**，尤其是自動遮蔽敏感資料的部分，遮蔽規則是簡單的格式比對（例如身分證字號格式），不保證 100% 抓到所有敏感內容

---

## 健保快易通資料匯入怎麼用

1. 打開「健保快易通」App → 健康存摺 → 下載你需要的資料（門診資料／用藥資料／影像或病理檢查資料／檢驗檢查結果），會得到 HTML 檔案
2. 到「資料匯入」頁面最上方的「健保快易通資料匯入」區塊，先選擇這批資料屬於「媽媽」還是「爸爸」
3. 把一個或多個 HTML 檔案拖進框框（可以一次選 4 個檔案一起匯入），系統會自動判斷每個檔案的類型
4. 按「解析健保資料」，系統會：
   - 把民國年日期自動轉換成西元年（例如 `112/05/30` → `2023-05-30`）
   - 依「醫事機構＋日期」把同一次看診/檢查的資料整理成一筆時間軸紀錄
   - 自動偵測並遮蔽報告內文中的姓名、病歷號碼、出生日期
   - 英文報告內容盡量附上中文參考翻譯（見下方說明）
   - 自動比對系統裡已經存在的資料，重複的項目會顯示「↺ 已存在，略過」並預設不勾選——所以之後不定期重新匯出、重新上傳，不會產生重複紀錄
5. 逐筆確認候選卡片內容（尤其是自動判斷的分類、家人），勾選要儲存的項目後按「儲存已勾選的紀錄」

**關於連續處方箋合併**：健保「慢性病連續處方箋」制度下，醫師一次看診可開立可調劑多次的處方，健保資料會把每次調劑各記一筆 claim，但「就醫日期」都寫開立處方那天，導致同一天可能出現好幾筆看起來重複的門診/用藥紀錄。系統會自動用「機構＋日期＋診斷碼」把這些合併成一張卡片，醫囑/用藥內容去重，並在卡片上標註「連續處方 N 次調劑」及每次的調劑效期，方便掌握下次領藥時間，同時大幅減少重複卡片。

**分類對應**：門診資料→就診、用藥資料→用藥、影像或病理檢查資料→檢查、檢驗檢查結果→檢查。所有健保快易通匯入的紀錄狀態預設為「已完成」（因為都是已發生的歷史資料），需要持續追蹤的項目可以自己改成「追蹤中」。

**關於科別／醫師欄位**：健保快易通匯出的門診資料、用藥資料、檢驗資料本身並不包含「科別」或「醫師姓名」這兩個欄位（只有醫事機構、診斷碼、藥品等），所以這幾類匯入不會顯示科別/醫師。只有「影像或病理檢查資料」的報告內文有時會附上開單醫師或報告醫師姓名，系統會盡量抓出來當作標籤。

**關於英文翻譯**：由於這是純前端網站、不能串接 AI 翻譯服務（原因跟不能用 GitHub Token 一樣——金鑰會暴露），系統改用「內建對照表」的方式：針對已經看過的報告內容整理好中文參考翻譯，遇到完全相符的英文段落就會在原文後面附上翻譯；遇到沒收錄過的新英文內容，會保留原文並在最上方標註「⚠️ 部分內容為英文原文，系統未能自動對照翻譯」提醒你。翻譯僅供參考，**正式醫療判斷仍請以醫師說明或原文報告為準**。之後如果匯入新的報告出現大量未翻譯的英文，可以把內容告訴我，我可以幫忙擴充對照表。

**關於重複匯入**：系統會替每一筆健保資料算出一組識別碼（醫事機構＋日期＋診斷碼/檢查代碼），下次重新匯入同一批資料時，只要識別碼相同就會被判斷為已存在。但如果健保資料本身在同一天有多筆相同診斷的claim（例如同一天看診同時申報好幾個項目），這些筆數在第一次匯入時仍會各自出現，可以自己勾掉不需要的重複項。

## 檔案結構

```
health-journal/
├── index.html          # 主頁面結構
├── css/style.css        # 樣式（Apple Health 風格設計系統）
├── js/
│   ├── firebase-config.js   # ← 你的 Firebase 設定要填在這裡
│   └── app.js                # 主程式邏輯
└── README.md
```
